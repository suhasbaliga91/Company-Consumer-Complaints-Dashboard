    ...
    return (
      <Card className="h-full bg-card border-border">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-semibold text-foreground">Dealership watchlist</CardTitle>
        </CardHeader>
        ...
        <div className="space-y-0.5">
          {pageItems.map((item, i) => (
            <div
              ...
              onClick={() => openDrawer(`Dealership: ${item.name}`, item.cases)}
            >
          ...
          {data.length === 0 && (
            <div className="flex items-center justify-center py-6 text-xs font-mono text-muted-foreground text-center px-4">
              No dealership data — run extract_respondents.py to populate
            </div>
          )}
        </div>
        {totalPages > 1 && (
