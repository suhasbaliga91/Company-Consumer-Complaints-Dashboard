...
The dashboard (`artifacts/dashboard`) has no access to the Python taxonomy
config — it only sees whatever fields the API returns — so it adapts to a
swapped taxonomy by looking at the actual case data instead of assuming
automotive categories:
- **Issue Mix** assigns bar colors dynamically to whatever `issue_type`
  values appear in your data, rather than a fixed automotive color map.
- **EV Overlay** and **Parts-Failure Tracker** are automotive-specific (`is_ev`,
  `part_category`/`part_involved`). Once extraction has run and those fields
  are still null for every case — meaning your taxonomy doesn't use them,
  not that extraction hasn't happened yet — the dashboard swaps those two
  panels out for **Grounds Analysis** and **Claimed vs Awarded**
  respectively, which work off fields (`grounds_taken`, `claim_amount`/
  `amount_awarded`) that apply to any industry. No configuration needed on
  the dashboard side.
